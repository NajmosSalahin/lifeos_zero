...
export interface IDashboardWidget { id: string; type: string; position: { x: number; y: number }; size: { w: number; h: number }; visible: boolean; pinned: boolean; }
export interface IUserPreferences { theme: string; font: string; fontSize: string; density: string; accentColor: string; sidebarCollapsed: boolean; sidebarWidth: number; reducedMotion: boolean; highContrast: boolean; readabilityMode: boolean; dashboardLayout: IDashboardWidget[]; }
export interface IUser extends Document {
  ...
  preferences: IUserPreferences; hydrationGoal: number; sleepGoal: number; timezone: string;
