...
export class AuthService {
  private cookieOpts(rememberMe: boolean) {
    return { httpOnly: true, secure: config.NODE_ENV === 'production', sameSite: 'strict' as const,
      maxAge: (rememberMe ? config.REMEMBER_ME_EXPIRES_DAYS : config.SESSION_EXPIRES_DAYS) * 86400000, path: '/' };
  }
  ...
  async register(dto: { email: string; password: string; firstName: string; lastName: string }) {
    ...
    const user = await User.create({ email: dto.email, passwordHash: dto.password, firstName: dto.firstName, lastName: dto.lastName, emailVerificationToken: hashToken(verificationToken), emailVerificationExpires: addDays(new Date(), 1) });
  ...
  async login(dto: { email: string; password: string; rememberMe?: boolean }, userAgent: string, ip: string) {
    ...
    user.lastLoginAt = new Date(); user.loginCount += 1; await user.save();
  ...
  async logout(rawToken: string) {
    await RefreshToken.findOneAndUpdate({ token: hashToken(rawToken) }, { isActive: false, revokedAt: new Date() });
  }

  async logoutAll(userId: string) {
    await RefreshToken.updateMany({ userId, isActive: true }, { isActive: false, revokedAt: new Date() });
  }
  ...
  async verifyEmail(token: string) {
    ...
    user.isEmailVerified = true; user.emailVerificationToken = null; user.emailVerificationExpires = null;
    await user.save(); return this.sanitize(user);
  }
  ...
  async resendVerification(userId: string) {
    ...
    user.emailVerificationToken = hashToken(token); user.emailVerificationExpires = addDays(new Date(), 1);
    await user.save(); await emailService.sendVerificationEmail(user.email, user.firstName, token);
  }
  ...
  async forgotPassword(email: string) {
    ...
    if (!user) return;
    const token = generateSecureToken();
    user.passwordResetToken = hashToken(token); user.passwordResetExpires = addHours(new Date(), 1);
    await user.save(); await emailService.sendPasswordResetEmail(user.email, user.firstName, token);
  }
  ...
  async resetPassword(token: string, newPassword: string) {
    ...
    user.passwordHash = newPassword; user.passwordResetToken = null; user.passwordResetExpires = null;
    await user.save(); await this.logoutAll(user._id.toString());
  }
